...
if ($('queueBtn')) $('queueBtn').onclick = () => vscode.postMessage({ type: 'queueComments' });
if ($('createProductBriefBtn')) $('createProductBriefBtn').onclick = () => vscode.postMessage({ type: 'createProductBrief' });
if ($('createPrdBtn')) $('createPrdBtn').onclick = () => vscode.postMessage({ type: 'createPrd' });
if ($('createArchitectureBtn')) $('createArchitectureBtn').onclick = () => vscode.postMessage({ type: 'createArchitecture' });
if ($('createTasksBtn')) $('createTasksBtn').onclick = () => vscode.postMessage({ type: 'createImplementationTasks' });
if ($('runQaBtn')) $('runQaBtn').onclick = () => vscode.postMessage({ type: 'runQaReview' });
/* v2.89.24 — 보고 스케줄 모달 */
...
function render(s) {
  ...
  /* Hidden legacy KPI strip: v0 dashboard focuses on product/project work. */
  if ($('kSubs')) $('kSubs').textContent = '–';
  if ($('kViews')) $('kViews').textContent = '–';
  if ($('kEng')) $('kEng').textContent = '–';
  animateNum($('kOpen'), s.tasks.open);
  $('kUrgent').textContent = s.tasks.urgent > 0 ? ('🔴 긴급 ' + s.tasks.urgent) : '';
  animateNum($('kApr'), s.approvals.length);

  /* Legacy content/analytics cards stay hidden in v0. */
  const ytConfigured = false;
  document.querySelectorAll('.yt-cond').forEach(el => { el.style.display = 'none'; });
  const ytBody = $('ytBody');
  if (!ytConfigured) {
  ...
  if (!s.oauthConnected) {
    anaBody.innerHTML = '<div class="ana-empty"><div class="icon">🔐</div><div class="msg">고급 분석 패널은 v0에서 비활성화되어 있습니다.</div></div>';
  ...
  if (s.yt && s.yt.configured && s.yt.myVideos && s.yt.myVideos.length > 0) {
    vBody.innerHTML = s.yt.myVideos.map(v =>
      '<a class="video-card" target="_blank" href="#">'
...
function showSkillDetail(agent, skill){
  ...
  /* v2.89.17 — 인앱 설정 폼. 도구의 .json + 공유 .json
  ...
  function renderConfigForm() {
    ...
    return '<div class="adm-skill-detail-form" id="admSkillForm">'
      ...
      + configFields.map((f, i) => {
          ...
          const sharedTag = f.source === 'shared' ? '<span class="adm-skill-detail-form-shared" title="공유 설정 파일: '+esc(skill.sharedConfigName||'shared')+'">🔗 공유</span>' : '';
          let inputHtml = '';
          if (isSelect) {
...
function showReportScheduleModal(entries){
  ...
  function toolSelect(curTool, curAgentId){
    /* v0는 소프트웨어 제작 워크플로에 집중한다. */
    const tools = [
      { agentId: 'planner', tool: 'prd_generator', label: '📋 PRD 초안 작성' },
      { agentId: 'architect', tool: 'adr_writer', label: '🏗️ ADR 초안 작성' },
      { agentId: 'frontend', tool: 'web_init', label: '🖥️ 프론트 프로젝트 초기화' },
      { agentId: 'qa', tool: 'test_plan', label: '✅ 테스트 계획 작성' },
    ];
...
/* v2.89.103 — 채용 PIN 모달. 잠긴 에이전트 카드 클릭 시 등장.
...
(function setupRevenueCard() {
  ...
  /* Legacy revenue widget is disabled in v0. */
...
function _renderRevenueMini(data) {
  ...
  if (!data || !data.totals) {
    document.getElementById('revSubtitle').textContent = '💡 매출 위젯은 v0에서 비활성화되어 있습니다.';
    return;
  }
